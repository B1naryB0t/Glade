...
function FollowersPage() {
  ...
  const { data: followersData, isLoading: loading, error } = useQuery({
    queryKey: ['followers', profileUsername],
    queryFn: () => userService.getFollowers(profileUsername),
    enabled: !!profileUsername,
  });

  const followers = followersData?.results || followersData || [];
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          <div className="bg-red-50 border-l-4 border-red-400 p-4 mb-4">
            <p className="text-sm text-red-700">
              {error?.message || 'Could not load followers. Please try again.'}
            </p>
          </div>
          <button 
            onClick={() => navigate(-1)}
          ...
          <div className="flex items-center mb-4">
            ...
            <div>
              <h1 className="text-4xl font-bold text-burgundy">
                {isOwnProfile ? 'Your Followers' : `${profileUser?.username}'s Followers`}
              </h1>
        ...
        {followers.length > 0 ? (
              ...
              <UserCard
                ...
                user={{
                  ...
                  location: follower.location ? 
                    `${follower.location.city}${follower.location.region ? `, ${follower.location.region}` : ''}` 
                    : null
                }}
          ...
          <div className="text-center py-12">
            ...
            <p className="mt-2 text-sm text-gray-500">
              {isOwnProfile 
                ? 'Start posting to gain followers!' 
