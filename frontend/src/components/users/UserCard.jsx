  ...
  return (
    <div className="bg-white rounded-lg shadow border border-gray-200 p-4 hover:shadow-md transition-shadow">
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3 flex-1 cursor-pointer" onClick={handleProfileClick}>
          {/* Avatar */}
      ...
      {/* Stats */}
      {(user.followers_count !== undefined || user.posts_count !== undefined) && (
        <div className="flex items-center space-x-4 mt-3 pt-3 border-t border-gray-100 text-sm text-gray-600">
          {user.posts_count !== undefined && (
            <span>
              <span className="font-semibold text-gray-900">{user.posts_count}</span> posts
            </span>
          )}
          {user.followers_count !== undefined && (
            <span>
              <span className="font-semibold text-gray-900">{user.followers_count}</span> followers
            </span>
          )}
          {user.following_count !== undefined && (
            <span>
              <span className="font-semibold text-gray-900">{user.following_count}</span> following
            </span>
          )}
        </div>
      )}
